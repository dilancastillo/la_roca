...
import { authSessionSchema, configuratorSessionSchema, loginRequestSchema, saveDesignRequestSchema, } from "@repo/shared/schemas/configurator";
import { authenticateUser, createSessionToken } from "./lib/auth.js";
import { getAppEnv, } from "./lib/app-env.js";
...
app.post("/design/save", zValidator("json", saveDesignRequestSchema), async (c) => {
    const appEnv = getAppEnv(c);
    ...
    try {
        ...
        return c.json({
            error: error instanceof Error ? error.message : "No se pudo guardar el diseno.",
        }, 400);
    }
});
export default app;
