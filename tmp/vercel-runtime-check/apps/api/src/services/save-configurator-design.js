...
function parseSelectedValueIds(selectedValueIds) {
    return Object.fromEntries(Object.entries(selectedValueIds).map(([key, values]) => [
        Number(key),
...
function validateSelections(session, selectedValueIds) {
    const errors = [];
    for (const attribute of session.attributes) {
        ...
        for (const valueId of selected) {
            if (!attribute.values.some((value) => value.id === valueId)) {
                errors.push(`El valor ${valueId} no pertenece al atributo "${attribute.name}".`);
            }
        }
    }
    const allSelectedIds = new Set(Object.values(selectedValueIds).flatMap((valueIds) => valueIds));
...
async function findExactVariant(env, session, variantValueIds) {
    const variants = await odooSearchRead(env, "product.product", [["product_tmpl_id", "=", session.productTemplateId]], ["id", "display_name", "product_template_attribute_value_ids"]);
    return variants.find((variant) => setsMatch(normalizeManyIds(variant.product_template_attribute_value_ids).sort((a, b) => a - b), [...variantValueIds].sort((a, b) => a - b)));
}
export async function saveConfiguratorDesign(env, payload) {
    ...
    if (!matchingVariant) {
        throw new Error("No existe una variante exacta para la combinacion seleccionada. Ajusta color, genero o talla y vuelve a intentar.");
    }
    ...
    const attachmentId = await odooCreate(env, "ir.attachment", [
        {
